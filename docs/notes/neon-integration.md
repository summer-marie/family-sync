...
If using Auth.js with Prisma, the database should also persist the standard auth models used for sessions and linked accounts.[3]
...
Google Calendar remains the source of truth for real calendar data.[4][5] The app database should store only the minimum needed to support the user experience: linked account identity, tokens through Auth.js/provider linkage, privacy settings, sync state, and any temporary cached schedule data needed for the shared view or AI answers.[4][5]
...
- Mixing app runtime DB access and migration/admin DB access carelessly in env config.
- Designing complex roles/permissions before the basic organizer/member flow works.
- Spending time on generalized calendar-provider abstractions for a Google-only MVP.

## Recommended implementation boundary for demo week

For demo week, Neon should cover:
...
- Privacy settings.
- Optional lightweight cached availability data.

It should not expand into unnecessary platform concerns such as file storage, realtime subscriptions, or multi-provider calendar abstractions this week.